...
CFLAGS_wasm_busyide = -Oz -s ERROR_ON_UNDEFINED_SYMBOLS=0 -lidbfs.js -s INVOKE_RUN=0 -s MODULARIZE=1 -s EXPORT_NAME=busbox -s FORCE_FILESYSTEM=1 -s EXPORTED_FUNCTIONS='[\"_main\",\"_fflush\"]' -s EXPORTED_RUNTIME_METHODS=[\"FS\",\"PATH\",\"callMain\",\"allocateUTF8OnStack\"] -s EXTRA_EXPORTED_RUNTIME_METHODS=[\"lengthBytesUTF8\"]
