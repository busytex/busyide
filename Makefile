...
build/wasm/busybox_unstripped.js: source/busybox.tar.bz2 source/miniz.zip openbsd_diff3prog.c openbsd_diff.c
	mkdir -p build/wasm/arch/em build/wasm
	...
	echo 'cmd_busybox__ = $$(CC) -o $$@.js -Wl,--start-group $(CFLAGS_wasm_busyide) $$(CFLAGS) $$(CFLAGS_busybox) $$(LDFLAGS) $$(EM_LDFLAGS) $$(EXTRA_LDFLAGS) $$(core-y) $$(libs-y) $$(patsubst %,-l%,$$(subst :, ,$$(LDLIBS))) -Wl,--end-group && cp $$@.js $$@' > build/wasm/arch/em/Makefile
...
versions.txt:
	...
	echo busybox: \\url{$(URL_busybox)} >> $@
	echo miniz: \\url{$(URL_miniz)} >> $@
	echo openbsd diff/diff3prog: \\url{$(URL_bsddiff)} >> $@
