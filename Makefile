...
URL_diff3prog ?= https://raw.githubusercontent.com/openbsd/src/master/usr.bin/diff3prog/diff3prog.c
...
CFLAGS_wasm_busyide = -Oz -s ERROR_ON_UNDEFINED_SYMBOLS=0 -lidbfs.js -s INVOKE_RUN=0 -s MODULARIZE=1 -s EXPORT_NAME=busbox -s FORCE_FILESYSTEM=1 -s EXPORTED_RUNTIME_METHODS=[\"FS\",\"PATH\",\"callMain\",\"allocateUTF8OnStack\"]
...
source/diff3prog.c:
	mkdir -p source
	wget "$(URL_diff3prog)" -O $@

build/wasm/busybox_unstripped.js: source/busybox.tar.bz2 source/miniz.zip source/diff3progprog.c
	...
	cat diff3prog.h > build/wasm/editors/diff3prog.c && sed 's/main/diff3prog_main/g' source/diff3progprog.c >> build/wasm/editors/diff3prog.c
