...
export class Shell
{
    constructor(monaco, ui, paths, readme, terminal, editor, difftool, cors_proxy_fmt = 'https://cors-anywhere.herokuapp.com/${url}')
    {
        ...
        this.busybox_applets = ['nanozip', 'bsddiff3prog', 'bsddiff', 'busybox', 'find', 'mkdir', 'pwd', 'ls', 'echo', 'cp', 'mv', 'rm', 'du', 'tar', 'touch', 'wc', 'cat', 'head', 'clear', 'unzip', 'gzip', 'base64', 'sha1sum', 'whoami'];
    ...
    async run(busybox_module_constructor, busybox_wasm_module_promise, sha1)
    {
        ...
        const sha1_ = uint8array => { console.log(this.busybox.run(['sha1sum'], uint8array).stdout); return sha1(uint8array); }; //this.FS.writeFile(this.tmp_file, uint8array) || this.busybox.run(['sha1sum', this.tmp_file]).stdout;
