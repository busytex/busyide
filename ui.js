...
export class Shell
{
    ...
    bind()
    {
        ...
        this.ui.share.onclick = () => this.commands(chain('cd', cmd('tar',  '-cf', this.shared_project_tar, this.PATH.basename(this.project_dir()), '--exclude', this.PATH.join2(this.PATH.basename(this.project_dir()), '.git')), cmd('cd', '-'), cmd('gzip', this.shared_project_tar), cmd('echo', '-n', this.ui.get_origin() + '/#base64targz/', '>', this.share_link_log), cmd('base64', '-w', '0', this.shared_project_targz, '>>', this.share_link_log), cmd('open', arg(this.share_link_log))));
